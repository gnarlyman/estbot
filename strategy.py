...
class StrategyA(engine.BaseEngine):
    ...
    def candle_open(self, candle):
        ...
        logger.debug('candle_open {:%m-%d-%Y %H:%M:%S}'.format(ts), extra=self.logger_extra)

    def candle_close(self, candle):
        ts = datetime.fromtimestamp(candle.time)
        logger.debug('candle_close {:%m-%d-%Y %H:%M:%S}'.format(ts), extra=self.logger_extra)
        ...
        if len(self.candle.candles) > 5:
            ...
            logger.debug('BUY_VOL_EMA: {}, SELL_VOL_EMA: {}'.format(buy_ema[-1], sell_ema[-1]), extra=self.logger_extra)
    ...
    def trend_price_up(self):
        logger.debug('trend_price_up: {}'.format(self.trend.curr_trend_price), extra=self.logger_extra)

    def trend_price_down(self):
        logger.debug('trend_price_down: {}'.format(self.trend.curr_trend_price), extra=self.logger_extra)

    def trend_retrace_up(self):
        logger.debug('trend_retrace_up: {} > {}'.format(self.trend.curr_price, self.trend.curr_trend_price), extra=self.logger_extra)

    def trend_retrace_down(self):
        logger.debug('trend_retrace_down: {} < {}'.format(self.trend.curr_price, self.trend.curr_trend_price), extra=self.logger_extra)
